...
def average_images(images):
    ...
    Assumes the images are already processed (and in a jet-colormap scheme).
    """
    if len(images) == 0:
        return None
    avg_img = np.zeros_like(images[0], dtype=np.float64)
    for img in images:
        avg_img += img / len(images)
    return np.round(avg_img).astype(np.uint8)

###########################
# PROCESSING FUNCTIONS (Otsu THRESHOLDING)
###########################
...
###########################
# VARIANCE CALCULATION: Compare each processed image to the average
###########################
def compute_difference_vs_average(processed_images, avg_img):
    """
    Given a list of processed images and their average image,
    compute the absolute difference between each processed image and the average.
    Returns a list of difference images.
    """
    diff_images = []
    for img in processed_images:
        diff = cv2.absdiff(img, avg_img)
...
def plot_and_save_average_image(avg_img, output_path, folder_name, axis_mode, mm_per_pixel_x, mm_per_pixel_y):
    """
    Displays the averaged image in a figure that:
      - Preserves the image's original pixel scale (no distortion),
      - Displays with (0,0) at the top-left,
      - Uses mm units if axis_mode=='mm' or pixels if axis_mode=='pixels',
      - Places a colorbar to the right whose height exactly matches the image height.
    
    The image is normalized (divided by 255) so that intensities are scaled to [0,1],
    and the ScalarMappable for the colorbar is set with vmin=0 and vmax=1.
    """
    if avg_img.ndim==3 and avg_img.shape[2]==3:
        gray_avg = cv2.cvtColor(avg_img, cv2.COLOR_BGR2GRAY)
    else:
        gray_avg = avg_img.copy()
    ...
    im = ax_img.imshow(norm_data, cmap='jet', origin='upper', vmin=0, vmax=1,
                       extent=extent, aspect='equal')
    ax_img.set_title(f"Averaged Difference Image: {folder_name}", pad=20)
...
###########################
# FOLDER PROCESSING FOR VARIANCE
###########################
def process_folders_for_variance(input_folder, output_folder, max_distance=50, axis_mode="pixels",
                                 mm_per_pixel_x=1.0, mm_per_pixel_y=1.0):
    """
    For each subfolder in input_folder, this function:
      - Loads and processes all images using Otsu thresholding,
      - Computes the average image of the processed images,
      - For each processed image, computes the absolute difference with the average image,
      - Averages these difference images to produce an averaged difference (variance) image,
      - Saves the raw averaged difference image and a corresponding graph (with the colormap normalized to 0-1).
    """
    ...
    for idx, folder_path in enumerate(valid_folders, start=1):
        ...
        image_files = sorted([f for f in os.listdir(folder_path) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff'))])
        processed_images = []
        ...
        if len(processed_images) < 1:
            print(f"  --> Not enough images in {folder_path} to compute variance.\n")
            continue
        
        # Compute average image from processed images
        avg_img = average_images(processed_images)
        # Compute difference of each processed image with the average image
        diff_images = []
        for img in processed_images:
            diff = cv2.absdiff(img, avg_img)
            diff_images.append(diff)
        # Average the difference images
        avg_diff = average_images(diff_images)
        
        relative_path = os.path.relpath(folder_path, input_folder).replace(os.sep, '_')
        raw_filename = f"avg_diff_image_{relative_path}.png"
        graph_filename = f"avg_diff_graph_{relative_path}.png"
        raw_path = os.path.join(output_folder, raw_filename)
        graph_path = os.path.join(output_folder, graph_filename)
        
        cv2.imwrite(raw_path, avg_diff)
        plot_and_save_average_image(avg_diff, graph_path, relative_path, axis_mode, mm_per_pixel_x, mm_per_pixel_y)
        print(f"  --> Raw averaged difference image saved to {raw_path}")
        print(f"  --> Graph of averaged difference image saved to {graph_path}\n")
...
if __name__ == "__main__":
    ...
    output_folder = "C:/Users/garci/Documents/GitHub/Spray-concentration-map/variance_otsu_results"
    process_folders_for_variance(input_folder, output_folder, max_distance=50,
