...
def plot_and_save_average_image(avg_img, output_path, folder_name, axis_mode, mm_per_pixel_x, mm_per_pixel_y):
    ...
    The averaged image is simply placed in the Matplotlib grid without reprocessing.
    For axis settings, if axis_mode=='mm', pixel coordinates are converted to mm.
    A colorbar is created using a separate ScalarMappable derived from the grayscale version
    of the averaged image (using the 'jet' colormap) so that the colorbar reflects the intensity range.
    """
    # If the image has 3 channels, assume it is already colored (in jet colormap) and use it directly.
    # For colorbar purposes, compute a grayscale version.
    if avg_img.ndim == 3 and avg_img.shape[2] == 3:
        image_to_show = avg_img.copy()
        gray_for_cbar = cv2.cvtColor(avg_img, cv2.COLOR_BGR2GRAY)
    else:
        image_to_show = avg_img.copy()
        gray_for_cbar = avg_img.copy()
    
    vmin = float(np.min(gray_for_cbar))
    vmax = float(np.max(gray_for_cbar))
    ...
    # Display the averaged image as is (no cmap parameter since it's already in jet scheme)
    im = ax_img.imshow(image_to_show, origin='upper', extent=extent, aspect='equal')
    ...
    # Create a ScalarMappable for the colorbar using the 'jet' colormap and the intensity range of the grayscale version.
