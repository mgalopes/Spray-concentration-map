...
class ImageProcessor:
    def __init__(self, 
        ...
        Parameters:
            ...
            output_root (str): Root directory to save colormaps and area images
    ...
    def _process_directory(self, dir_path: str, files: List[str]) -> Optional[str]:
        ...
        # === Ratio Calculation Restricted to the Main Object ===
        # Normalize final_map to 0-1 range
        normalized_final = cv2.normalize(final_map, None, 0, 1, cv2.NORM_MINMAX)
        # Retrieve the main object mask from the binary map
        ...
        # Total area of the main object (number of pixels)
        main_object_area = np.count_nonzero(main_object_mask)
        
        # First, define the fixed and variable masks based on thresholds:
        #   - Fixed: pixels with normalized value >= 0.95
        #   - Variable: pixels with normalized value between 0.05 and 0.95
        ...
        # There might be pixels in the main object that do not fall into either category.
        # We add those pixels to the variable area to ensure the partition covers the entire main object.
        ...
        # Sanity check: total_partition should equal main_object_area.
        if total_partition != main_object_area:
            print(f"  ⚠ Warning: Partitioned area ({total_partition}) does not equal main object area ({main_object_area})")
        else:
        ...
        # === Save Fixed and Variable Area Images ===
        # Create images showing only the fixed or variable areas (all other pixels set to 0)
        ...
        fixed_area_save = (fixed_area_image * 255).astype(np.uint8)
        variable_area_save = (variable_area_image * 255).astype(np.uint8)
        
        fixed_area_filename = f"{dir_name}_fixed_area.png"
        fixed_area_path = os.path.join(self.output_root, fixed_area_filename)
        variable_area_filename = f"{dir_name}_variable_area.png"
        variable_area_path = os.path.join(self.output_root, variable_area_filename)
        
        cv2.imwrite(fixed_area_path, fixed_area_save)
        cv2.imwrite(variable_area_path, variable_area_save)
        print(f"  ✓ Saved fixed area image: {fixed_area_path}")
        print(f"  ✓ Saved variable area image: {variable_area_path}")
        # === End Saving Area Images ===
